...
The popultation of interest in this study is first year undergraduate collge students. At a more abstract level, my population of interest is all undergraduate students who take a general education English course In the end, the comparison I want to show is that students who take these classes do, in fact, improve or learn.

There are four outcome variables of interests in this dataset: the micro, mid, macro, and overall quality of all students' papers. This study is a within-subjects design as I have all the students first and last papers of the semester. The first paper for all students functions as a pretreatment condition. Since this was the first writing assignment all students completed, no other written comments from professors confounded these initial assessments of the overall groups. At the end of the day, I'm interested in seeing if the overall group of students improved their writing ability, so I want to calculate the overall means for all of the students' first papers. Then, I would want to calculate the means of the students' second papers and look at the average difference in the means after the treatment, or written comments, were provided. Given my research question, I decided to use the paired-sample t-test to answer my research question. Most researchers use a paired sample t-test when they want to compare two sets of measurment to see how group means are different. Given my small sample size of n=26, one additional benefit of this test is the increased statistical power. Such a "repeated measures" test compares these measurements within subjects, rather than across subjects, and will generally have greater power than an unpaired test.

I chose to combine all of the students together in the analysis for two reasons. First, my primary research question is seeing if any discernable effect is occuring from professors' comments on student writing. Although the professors comment in a different way, these comments can both be considered treatments. Ideally, students in both classes after they are pooled together show improvement. Second, the two student groups are similar in a variety of ways. They are all first-year students, they are all taking an introductory English course, and they are all in their second semester of undergrad. The student populations shoudld largely be homogenous groups. 

Since I am interested in the differences between the two group means, I decided to conduct a paired sample t-test with alpha=0.05. There are a few assumptions regarding the paired sample t-test: 1). The observations are independent of one another. 2). The dependent variable should not include outliers. (citation). In this case, it is reasonable for us to assume that the observations are independent of another. In human speak, this simply means that one students overall score on a paper does not influence another student's overall score. Further, there are no outliers in my dependent variable. All students received similar scores to one another. Finally, I can confidently say that the variences are assumed to be equal to one another. [Include more information about the assumptions of the paried sample t-test and justify your choice more to use this test.]
...
But, this is only one p-value for both outcome variables overall and micro. I want to be especially cautious and run a permutation test of all my different variables. Why am I running a permutation based test? The permutation test is a nonparametric way to test for differences between my samples. One advantage to this approach is that I do not need to assume the type of distribution my data follows. If the data did not follow a normal distribution, and I conducted my analysis the results could be confounded by this initial assumption. The second advantage of this approach is that I test all possible combinitations of my control and treatment groups with my given data, so I can be more confident that the effects I see are not due to chance. [Need to include more explanation of permutation based testing.]
```{r, results="hide", include=FALSE}
...
```
After running the permutation test for my varibles overall and macro, I found a p-value of 0.004 for overall and 0.01 for macro level, so I reject the null hypotheses. These results suggest that students are improving the quality of their overall papers, and these effects are most likely not due to chance. After running the paired sample t-test and permutation test, it is still possible that I am commiting a type I error and the effects we see in student improvement are false positives. However, running both tests gives me more confidence that I am less likely to comit a type I error with my data. 

P-values can be deceptive though! A p-value alone does indicate evidence against our null hypothesis, but it doesn't tell us anything about the effect size we observe. A p-value, after all, doesn't capture the effect size of my variables. So, what are the effect sizes for overall and macro? In order to calculate the effect sizes of my outcomes varaibles, I decided to find Cohen's d. One reason I decided to use Cohen's d was to calculate the effect sizes from the difference in means between the students' first and second papers. [Need to include more discussion of Cohen's d here.]
...
====In this next section, I will calculate the error rate for my overall and macro level outcomes variables. I'm still working on this at this point, but this process should also show that my findings are even more robust. >:(. I wish the R code worked. 
...
##Purpose
#====5 & 6 are NA's. Need to remove. 
counts1 <- table(s15comments$Class, s15comments$Purpose)
...
##Style 
counts2 <- table(s15comments$Class, s15comments$Style)
...
When creating my barplots, I was unable to change the legend for all of them--I'll work on fixing this later. [I had ggplot graphics, but they aren't working. >:(. I'll talk to Ekrem or Lula later. So frustrating. Instead of having "1" on the x-axis, it should be listing the corresponding type of comment/style listed in the appendix.] The blue #1 respresents the first class and the red #2 represents the second class. I will also work on fixing the legend for the last graph I present. So far, this is solely descriptive analysis across four different variables of interest that function as a type of treatment. This analysis fuses paper 1 & 2 as well as paper 3 & 4. This is simply meant to demonstrate the most common approaches that both professors took in making comments about their papers. 
...
counts <- as.matrix(temp1$TotStyle1, temp1$TotStyle2, temp1$TotStyle3, temp1$TotStyle4)
class(counts)
barplot(counts, main="Car Distribution by Gears and VS",
  xlab="Number of Gears", col=c("darkblue","red", "green", "blue"),
 	legend = rownames(counts), beside=TRUE)
ggplot(s15comments, aes(Style, fill=as.factor(Purpose)))+geom_bar(position="dodge")

##=====Try making widths of histogram the same. 

s15comments2 %>% filter(Style!="NA" & Purpose != "NA") %>%
  ggplot(aes(Style, fill=Purpose))+
  geom_bar(position="dodge")+
  facet_wrap(~Class, ncol=2)+
  ggtitle("Overall Professor Comments")+
  theme_bw()+
...
s15comments2 %>% filter(Style!="NA" & Purpose != "NA") %>%
  ...
  count(Style) %>% 
  ggplot(aes(Style, n, fill=Purpose))+
