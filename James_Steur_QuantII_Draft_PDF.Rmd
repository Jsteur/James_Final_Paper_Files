...
I found an effect size of 0.71 for overall and 0.61 for macro (Table 2), but what do these effect sizes really mean? Conventionally, an effect size of 0.8 is considered large, 0.5 is medium, and 0.2 is small. Put differently, seeing an effect size of 0.2 is usually something you can only see with careful study, whereas an effect size of 0.8 is high enough for someone to see the effects with the naked eye. The effect size largely depends on context. For example, seeing an effect size of 0.2 may usually be considered small, but it may be large if the effect is observed in preventing suicide. Context and expertise is important when interpreting the different effect sizes. 

Given my small sample size, these effect sizes are fairly large. This means that students appear to be improving at a meaningful rate with their writing abilities by the end of the semester. These effect sizes indicate that students are learning over the course of the semetser and end up producing "better" quality work. 
...
After I calculated the error rate, I decided to calculate the power rate of my overall and macro level outcome variables. The power rate is going to assess the probability of me making a type II error given the size of my data set. The three factors that influence power are my alpha value, effect size, and sample size that I use to detect the effect size. In order to be especially cautious, I ran 1000 different simulations of my data and found each of those power rates. Afterwards, I took the mean of all my power rates. Rather than just run one simulation, running 1000 gives me more confidence that my power rate is more likely to be accurate. 
